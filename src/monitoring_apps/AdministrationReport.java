...
public class AdministrationReport extends javax.swing.JFrame {

    private final Connection conn = new KoneksiDb().connect();
    private DefaultTableModel tabmode;

    public AdministrationReport() {
        initComponents();
    ...
    private void initUi() {
        pageTitle1.setText("REPORT ADMINISTRATION");
    ...
    private void initTable() {
        Object[] columns = new Object[]{"#", "Tipe Surat", "Nama Surat", "No Surat", "Pengirim", "Penerima", "Tgl Surat", "Project ID", "Project Name"};
        tblProjectList.setTableData(new Object[0][columns.length], columns);
        tabmode = (DefaultTableModel) tblProjectList.getModel();
    }

    private void generateReport() {
        loadAdministration(txtPeriode.getText());
    }


    private void loadAdministration(String periode) {
        Object[] columns = new Object[]{"#","Tipe Surat","Nama Surat","No Surat","Pengirim","Penerima","Tgl Surat","Project ID","Project Name"};
        tblProjectList.setTableData(new Object[0][columns.length], columns);
        tabmode = (DefaultTableModel) tblProjectList.getModel();
        ...
        String[] queries = new String[]{
            "SELECT a.tipe AS tipe_surat, a.nama AS nama_surat, a.no_surat, a.pengirim, a.penerima, a.tgl_surat, a.project_id, COALESCE(p.nama, '') AS project_name "
                + "FROM administrasi a LEFT JOIN project p ON p.id = a.project_id "
                + "WHERE (? = '' OR a.tipe LIKE ? OR a.nama LIKE ? OR a.no_surat LIKE ? OR a.pengirim LIKE ? OR a.penerima LIKE ? OR CAST(a.tgl_surat AS CHAR) LIKE ? OR CAST(a.project_id AS CHAR) LIKE ? OR p.nama LIKE ?) "
        ...
        for (String query : queries) {
            try (PreparedStatement ps = conn.prepareStatement(query)) {
                for (int i = 1; i <= 9; i++) {
                    ps.setString(i, i == 1 ? search : like);
                }
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    ...
                    while (rs.next()) {
                        tabmode.addRow(new Object[]{
                            no++,
                            safe(rs.getObject("tipe_surat")),
                            safe(rs.getObject("nama_surat")),
                            safe(rs.getObject("no_surat")),
                            safe(rs.getObject("pengirim")),
                            safe(rs.getObject("penerima")),
                            safe(rs.getObject("tgl_surat")),
                            safe(rs.getObject("project_id")),
        ...
        showError("Gagal mengambil data administrasi. Sesuaikan nama tabel/kolom administration.\n" + (lastError == null ? "" : lastError.getMessage()));
    }
    ...
    private void setColumnWidths() {
        ...
        int[] widths = {45, 95, 140, 115, 110, 110, 100, 85, 160};
    ...
    private void exportReport() {
        ...
        chooser.setDialogTitle("Simpan REPORT ADMINISTRATION");
        chooser.setSelectedFile(new File("ReportAdministration.pdf"));
        ...
        try {
            InputStream reportStream = getClass().getResourceAsStream("/report/AdministrationReport.jrxml");
            if (reportStream == null) {
                throw new IllegalStateException("File /report/AdministrationReport.jrxml tidak ditemukan di folder src/report.");
            }
            ...
            parameters.put("summary2", "Periode Filter : " + (txtPeriode.getText() == null ? "" : txtPeriode.getText().trim()));

            List<Map<String, Object>> dataList = new ArrayList<>();
            for (int r = 0; r < tabmode.getRowCount(); r++) {
                Map<String, Object> row = new HashMap<>();
                row.put("no", safe(tabmode.getValueAt(r, 0)));
                row.put("tipe_surat", safe(tabmode.getValueAt(r, 1)));
                row.put("nama_surat", safe(tabmode.getValueAt(r, 2)));
                row.put("no_surat", safe(tabmode.getValueAt(r, 3)));
                row.put("pengirim", safe(tabmode.getValueAt(r, 4)));
                row.put("penerima", safe(tabmode.getValueAt(r, 5)));
                row.put("tgl_surat", safe(tabmode.getValueAt(r, 6)));
                row.put("project_id", safe(tabmode.getValueAt(r, 7)));
                row.put("project_name", safe(tabmode.getValueAt(r, 8)));
                dataList.add(row);
            }
    ...
    public static void main(String args[]) {
        try {
            ...
            java.util.logging.Logger.getLogger(AdministrationReport.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }

        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new AdministrationReport().setVisible(true);
            }
        });
    }
